...
FolderDirect <- '/Users/andremac/Documents/Sophie/Teste final/Data and analysis'
out <- ('/Users/andremac/Documents/Sophie/Teste final/Data and analysis/Output')
setwd(FolderDirect)

# Load the dataset
gamedata <- read_dta('/Users/andremac/Documents/Sophie/Teste final/Data and analysis/datagame.dta')
head(gamedata)
colnames(gamedata)
...
# Sum player columns to create 'cointosses'
gamedata <- gamedata %>%
  mutate(
    cointosses = playerl1 + playerl2 + playerl3 + playerl4 + playerl5 + playerl6,
    no_cointosses = 6 - cointosses,
    lossavers = no_cointosses / 6
  )

# Display lossavers values when period == 1
table(gamedata$lossavers[gamedata$period == 1])

# Initialize *_t columns to 0 for monotonicity check
for (var in paste0("playerl", 1:6)) {
  gamedata[[paste0(var, "_t")]] <- 0
}

# Update *_t columns based on condition in reverse order
for (i in 6:1) {
  for (var in paste0("playerl", 1:i)) {
    gamedata[[paste0(var, "_t")]] <- ifelse(gamedata[[paste0("playerl", i)]] == 1, 1, gamedata[[paste0(var, "_t")]])
  }
}

# Sum updated *_t columns to create 'cointosses_t' and check for errors in monotonicity
gamedata <- gamedata %>%
  mutate(
    cointosses_t = playerl1_t + playerl2_t + playerl3_t + playerl4_t + playerl5_t + playerl6_t,
    Fehler = ifelse(cointosses != cointosses_t, 1, 0),
    lossavers_rel_orig = lossavers
  )

# Remove 'lossavers' values if monotonicity is violated
gamedata <- gamedata %>%
  mutate(lossavers = ifelse(Fehler == 1, NA, lossavers))

# Define 'weird_LossAversion' and 'mulSw_LossAversion' based on conditions
gamedata <- gamedata %>%
  mutate(
    weird_LossAversion = ifelse(Fehler == 1 & id != 155, 1, 0),
    mulSw_LossAversion = ifelse(id == 155, 1, 0)
  )

# Create 'lossaversion_ordinal' and assign specific values if certain conditions are met
gamedata <- gamedata %>%
  mutate(
    lossaversion_ordinal = no_cointosses,
...
write.csv(gamedata, file = file.path(out, "dadosdojogo.csv"), row.names = FALSE)


dados <- read.csv('/Users/andremac/Documents/Sophie/Teste final/Data and analysis/Output/dadosdojogo.csv')
